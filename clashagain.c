...
TPlayer initPlayer(Tcouleur couleur){
    ...
    p.elixir=20; //nombre_aleatoire(2,10);
    return p;
};
...
void deplacement (TplateauJeuUpgrade jeuEncours , int largeur , int hauteur ){
    ...
    for(int i=0; i<largeur ; i++){
        for(int j=0 ; j<hauteur ; j++){
            ...
            tmp=list;
            while(tmp != NULL ){
                jeuEncours[i][j]= supprimerUnitedansZone (jeuEncours[i][j] ,tmp->pdata );
                tmp=tmp->suiv;
            }
            while(list != NULL){
...
void sauvgardeSEQ (FILE *fichier , TPlayer player1 , TPlayer player2){
    ...
    tmp=player1.listeU;
    fprintf(fichier,"%d ",player1.couleurJoueur);
    fprintf(fichier,"%d ",player1.elixir);
    while( tmp != NULL){
        unite= tmp->pdata;
        fprintf(fichier,"%d ",unite->nom);
        fprintf(fichier,"%d ",unite->pointsDeVie);
        fprintf(fichier,"%d ",unite->posX);
        fprintf(fichier,"%d ",unite->posY);
        fprintf(fichier,"%d ",unite->peutAttaquer);
        tmp=tmp->suiv;

    }

    tmp=player2.listeU;
    fprintf(fichier,"%d ",player2.couleurJoueur);
    fprintf(fichier,"%d ",player2.elixir);
    while( tmp != NULL){
        unite= tmp->pdata;
        fprintf(fichier,"%d ",unite->nom);
        fprintf(fichier,"%d ",unite->pointsDeVie);
...
void restaurSEQ (FILE* fichier ,TPlayer* p1 , TPlayer* p2){
    ...
    fscanf(fichier,"%d",&taille1);
    fscanf(fichier,"%d",&taille2);
    /*player 1*/
    fscanf(fichier,"%d",&nb);
    switch(nb){
        case 0 : color = rouge ; break ;
        case 1 : color = bleu  ; break ;
    }
    p1->couleurJoueur = color;
    fscanf(fichier,"%d",&nb);
    p1->elixir = nb;
    ...
    for(int i=0; i<taille1 ; i++ ){
        fscanf(fichier,"%d",&nb);
        switch(nb){
            case 0 : unite =creeTour(0,0); break ;
            case 1 : unite =creeTourRoi(0,0); break;
            case 2 : unite =creeArcher(0,0); break;
            case 3 : unite =creeChevalier(0,0); break;
            case 4 : unite =creeDragon(0,0); break;
            case 5 : unite =creeGargouille(0,0); break;
        }
        unite->couleur= color;
        printf("on ajout dans la restaure ");
        p1->listeU = AjouterUnite(p1->listeU ,unite);
        fscanf(fichier,"%d",&nb);
        p1->listeU->pdata->pointsDeVie = nb ;
        fscanf(fichier,"%d",&nb);
        p1->listeU->pdata->posX = nb ;
        fscanf(fichier,"%d",&nb);
        p1->listeU->pdata->posY = nb ;
        fscanf(fichier,"%d",&nb);
        p1->listeU->pdata->peutAttaquer = nb ;
    ...
    for(int i=0; i<taille1 ; i++ ){
        ...
        unite->couleur=color;
        p2->listeU = AjouterUnite(p2->listeU ,unite);
        fscanf(fichier,"%d",&nb);
        p2->listeU->pdata->pointsDeVie = nb ;
        fscanf(fichier,"%d",&nb);
        p2->listeU->pdata->posX = nb ;
        fscanf(fichier,"%d",&nb);
        p2->listeU->pdata->posY = nb ;
        fscanf(fichier,"%d",&nb);
        p2->listeU->pdata->peutAttaquer = nb ;
